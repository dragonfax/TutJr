...
void Door::draw() {
  ...
  if ( doors & DOOR_UP ) {
    ...
    arduboy.drawFastVLine( (center.x) * CELL_SIZE + CELL_SIZE/2, (center.y - 2)* CELL_SIZE, 8, 1);
  }

  if ( doors & DOOR_DOWN ) {
    // mvaddch(center.y + 1, center.x, 'D');
    // mvaddch(center.y + 2, center.x, 'D');
    arduboy.drawFastVLine( (center.x) * CELL_SIZE + CELL_SIZE/2, (center.y + 1)* CELL_SIZE, 8, 1);
  }

  if ( doors & DOOR_LEFT ) {
    // mvaddch(center.y, center.x - 1, 'D');
    // mvaddch(center.y, center.x - 2, 'D');
    arduboy.drawFastHLine( (center.x - 2) * CELL_SIZE, (center.y)* CELL_SIZE + CELL_SIZE/2, 8, 1);
  }

  if ( doors & DOOR_RIGHT ) {
    // mvaddch(center.y, center.x + 1, 'D');
    // mvaddch(center.y, center.x + 2, 'D');
    arduboy.drawFastHLine( (center.x + 1) * CELL_SIZE, (center.y)* CELL_SIZE + CELL_SIZE/2, 8, 1);
  }
}
